...
export class LoginHandler {
    ...
    validarLogin() {
        ...
        if (userObj && passInput === userObj.pass.toLowerCase()) {
            ...
            // Configurar filtros según el usuario
            const filters = {};
            if (userObj.pais !== 'TODOS') filters.countries = [userObj.pais];
            if (userObj.division !== 'TODOS') filters.divisions = [userObj.division];
            if (userObj.grupo !== 'TODOS') filters.groups = [userObj.grupo];
...
// Exponer métodos para onclick en HTML
window.loginHandler = {
    seleccionarPais: (c, n) => window.app?.loginHandler?.seleccionarPais(c, n),
    seleccionarAccesoRegional: () => window.app?.loginHandler?.seleccionarAccesoRegional(),
    volverAPasoPais: () => window.app?.loginHandler?.volverAPasoPais(),
    volverDesdeLogin: () => window.app?.loginHandler?.volverDesdeLogin()
};
